...
const pricingOptions = [
  {
    ...
    features: [
      ...
      "Vitaminas A, E, B1, B6, B12 + Biotina",
      "Envío GRATIS por Coordinadora",
      "Registro INVIMA certificado",
...
export function PricingSection() {
  ...
  return (
    <>
        ...
        <div className="container mx-auto px-4">
          <div className="text-center mb-12 md:mb-16">
            ...
            <h2 className="text-3xl md:text-4xl lg:text-5xl font-bold mb-4 text-balance">
              Elige Tu Plan y Empieza a Ver <span className="text-primary">Resultados</span>
            </h2>
            <p className="text-base md:text-lg text-muted-foreground max-w-2xl mx-auto text-pretty">
              Envío gratis a toda Colombia por Coordinadora. Pago 100% seguro. Garantía de satisfacción.
            </p>
          </div>
                ...
                <CardHeader className="text-center pb-4 pt-6 md:pt-8">
                  ...
                  <div className="mb-3">
                    <div className="text-sm text-muted-foreground mb-1">
                      Antes: <span className="line-through">${option.originalPrice.toLocaleString("es-CO")}</span>
                    </div>
                  ...
                  <div className="flex flex-col gap-2">
                    <Badge
                      variant="outline"
                      className="w-fit mx-auto border-accent/50 text-accent-foreground bg-accent/10 rounded-full text-xs md:text-sm px-3 py-1"
                    >
                      Ahorras ${option.savings.toLocaleString("es-CO")}
                    </Badge>
                    <Badge className="w-fit mx-auto bg-primary/10 text-primary border-primary/30 rounded-full text-xs">
                  ...
                  <div className="space-y-2 md:space-y-3">
                    {option.features.map((feature, index) => (
                      <div key={index} className="flex items-start gap-2 md:gap-3">
                        ...
                        <span className="text-xs md:text-sm leading-relaxed text-foreground">{feature}</span>
                      </div>
                    ))}
                  </div>

                  <div className="bg-accent/10 rounded-xl p-3 md:p-4 space-y-2 border border-accent/20">
                    ...
                    <div className="flex items-center gap-2 text-xs md:text-sm">
                      ...
                      <span className="font-medium text-foreground">Cabello más fuerte y uñas duras</span>
                    </div>
                  </div>
      ...
      {selectedOption && (
        <CheckoutModal isOpen={isCheckoutOpen} onClose={() => setIsCheckoutOpen(false)} product={selectedOption} />
      )}
    </>
  )
}
